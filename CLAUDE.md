...
**shipstatic** — VS Code extension that registers the ShipStatic MCP server as a VS Code MCP provider, making all 15 tools available in GitHub Copilot's agent mode. Also provides deploy, whoami, and API key commands. Published to the VS Code Marketplace. **Maturity:** v0.1.x — MCP provider + basic commands.
...
pnpm test --run     # All tests (30 tests, ~350ms)
...
The extension registers a `McpServerDefinitionProvider` via `vscode.lm.registerMcpServerDefinitionProvider()`. This is the VS Code 1.99+ API for auto-discovering MCP servers in Copilot's agent mode.
...
2. `resolveMcpServerDefinition()` — Called when the server is about to **start**. Prompts for API key via `ensureApiKey()`. Sets `SHIP_API_KEY` env var on the server definition.
...
`onDidChangeMcpServerDefinitions` event fires after `setApiKey` command so VS Code re-queries the provider.
...
├── auth.test.ts         # SecretStorage flows (7 tests)
├── mcp.test.ts          # Provider registration + resolve lifecycle (9 tests)
├── commands.test.ts     # All 3 commands + SDK arg verification (11 tests)
