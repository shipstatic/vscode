...
/** Built from the shape constants — passes the platform's own validator. */
const TEST_API_KEY = `${API_KEY.PREFIX}${'a'.repeat(API_KEY.HEX_LENGTH)}`;
/** The other half of the one slot, same width by the shape law. */
const TEST_DEPLOY_TOKEN = `${DEPLOY_TOKEN.PREFIX}${'b'.repeat(DEPLOY_TOKEN.HEX_LENGTH)}`;
...
describe('auth', () => {
  ...
  describe('getToken', () => {
    it('returns the stored token', async () => {
      ...
      expect(await getToken(ctx)).toBe(TEST_API_KEY);
    });
    ...
    it('does not fall back to the 0.2.x key', async () => {
      ...
      await ctx.secrets.store(LEGACY_KEY, TEST_API_KEY);
      expect(await getToken(ctx)).toBeUndefined();
    });
  });
  ...
  describe('setToken', () => {
    ...
    describe('validateInput — the platform validator, not a local rule', () => {
      ...
      it('accepts a well-formed API key', async () => {
        expect((await validator())(TEST_API_KEY)).toBeNull();
      });
      ...
      it('accepts a well-formed deploy token', async () => {
        ...
        expect((await validator())(TEST_DEPLOY_TOKEN)).toBeNull();
      });
  ...
  describe('migrateSecret', () => {
    it('moves a 0.2.x credential onto the one-slot key and drops the old one', async () => {
      ...
      expect(await ctx.secrets.get(KEY)).toBe(TEST_API_KEY);
    ...
    it('keeps the newer credential when both keys hold one', async () => {
      ...
      await ctx.secrets.store(LEGACY_KEY, TEST_API_KEY);
      await ctx.secrets.store(KEY, TEST_DEPLOY_TOKEN);

      await migrateSecret(ctx);

      expect(await ctx.secrets.get(KEY)).toBe(TEST_DEPLOY_TOKEN);
      expect(await ctx.secrets.get(LEGACY_KEY)).toBeUndefined();
    });
